...
export function MobileWealthGoalsAssetsPage() {
  ...
  const applyManualClientId = () => {
    ...
    setSuccessMessage(t.mobileWealth.clientIdSaved);
    setError(null);
  };
  ...
  const handleAddAsset = async () => {
    if (!selectedClientId) {
      setError(toApiError(new Error(t.mobileWealth.errorSelectClient)));
      return;
    }
    const value = Number(newAssetValue);
    if (!Number.isFinite(value) || value <= 0) {
      setError(toApiError(new Error(t.mobileWealth.errorAssetValue)));
      return;
    }
    setSubmittingAsset(true);
    try {
      await wealthApi.createAsset(selectedClientId, { assetType: newAssetType, value });
      setSuccessMessage(t.mobileWealth.assetAdded);
      setNewAssetValue('');
      await refreshLists();
    } catch (err) {
      setError(toApiError(err));
  ...
  const handleAddGoal = async () => {
    ...
    if (!Number.isFinite(targetAmount) || targetAmount <= 0) {
      setError(toApiError(new Error(t.mobileWealth.errorGoalAmount)));
      return;
    }
    setSubmittingGoal(true);
    try {
      await wealthApi.createGoal(selectedClientId, { goalType: newGoalType, targetAmount });
      setSuccessMessage(t.mobileWealth.goalAdded);
      setNewGoalAmount('');
      await refreshLists();
    } catch (err) {
      setError(toApiError(err));
  ...
  return (
    ...
    >
      ...
      <div className="rounded-3xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-5 shadow-sm">
        <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-400">{t.mobileWealth.declaredHoldings}</p>
        <p className="mt-1 text-3xl font-bold tracking-tight text-zinc-900">${formatMoney(totalAssets)}</p>
        <p className="mt-1 text-xs text-zinc-500">
          {loadingData
            ? t.mobileWealth.refreshing
            : t.mobileWealth.assetsAndGoals
                .replace('{assets}', String(assets.length))
                .replace('{goals}', String(goals.length))}
        </p>
      </div>
      ...
      {!loadingClients && clientOptions.length === 0 && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50/90 p-4 space-y-3">
          <p className="text-xs text-amber-900 font-medium leading-relaxed">
            {t.mobileWealth.clientModeHint}
          </p>
          <div className="flex gap-2">
            <input
              type="text"
              placeholder={t.mobileWealth.clientIdPlaceholder}
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 ml-1">{t.mobileWealth.clientLabel}</label>
        <div className="flex gap-2">
          <select
          ...
          >
            {clientOptions.length === 0 && <option value="">{t.mobileWealth.clientIdHint}</option>}
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-tight">{t.mobileWealth.assetsTitle}</h3>
        <div className="space-y-2 rounded-2xl border border-zinc-100 bg-zinc-50/50 p-4">
          {assets.length === 0 && !loadingData && (
            <p className="text-xs text-zinc-500">{t.mobileWealth.noAssets}</p>
          )}
          {assets.map((row) => (
            <div
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <input
            type="number"
            placeholder={t.mobileWealth.valuePlaceholder}
            value={newAssetValue}
        ...
        >
          ...
          {submittingAsset ? t.mobileWealth.saving : t.mobileWealth.addAsset}
        </button>
      </section>

      <section className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-tight">{t.mobileWealth.goalsTitle}</h3>
        <div className="space-y-2 rounded-2xl border border-zinc-100 bg-zinc-50/50 p-4">
          {goals.length === 0 && !loadingData && (
            <p className="text-xs text-zinc-500">{t.mobileWealth.noGoals}</p>
          )}
          {goals.map((row) => (
            <div
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <input
            type="number"
            placeholder={t.mobileWealth.targetAmountPlaceholder}
            value={newGoalAmount}
        ...
        >
          <Plus className="w-4 h-4" />
          {submittingGoal ? t.mobileWealth.saving : t.mobileWealth.addGoal}
        </button>
      </section>
    </motion.div>
  );
